        ...
        <div className="space-y-4">
          ...
          <div className="bg-card rounded-2xl border border-border p-5 sm:p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h1 className="text-xl font-heading font-extrabold text-foreground">{selectedClient.name}</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 text-sm">
              ...
              <div><span className="text-muted-foreground">Since:</span> <span className="font-medium text-foreground">{selectedClient.createdAt}</span></div>
            </div>
          </div>
                ...
                {clientProjects.map((p) => (
                  <Link key={p.id} to={`/admin/projects/${p.id}`} className="px-4 py-3.5 flex items-center justify-between hover:bg-muted/20 transition-colors">
                    <div>
                      ...
                      <div className="mt-1"><StatusBadge status={p.buildStage} /></div>
                    </div>
