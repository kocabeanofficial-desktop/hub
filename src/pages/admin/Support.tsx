      ...
      <div className="space-y-4">
        ...
        <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm">
                ...
                {supportTickets.map((t) => (
                  <tr key={t.id} className="hover:bg-muted/20 transition-colors">
                    <td className="px-4 py-3.5">
                      ...
                      <p className="text-xs text-muted-foreground sm:hidden mt-0.5">{getClientName(t.client_id)}</p>
                    </td>
                    <td className="px-4 py-3.5 text-muted-foreground hidden sm:table-cell">{getClientName(t.client_id)}</td>
          ...
          {!isLoading && supportTickets.length === 0 && (
            <div className="px-4 py-12 text-center text-sm text-muted-foreground">No support tickets.</div>
          )}
        </div>
      </div>
    </DashboardLayout>
